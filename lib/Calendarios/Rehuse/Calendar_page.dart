...
class __CalendarViewState extends State<_CalendarView> {
  ...
  Future<Meeting> getRecurrenceData(Appointment appointment) async {
    ...
    var snapshot = await databaseReference
        .collection("Appoints")
        .where(FieldPath.documentId, isEqualTo: appointment.id.toString())
        .get();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: CustomDrawer(
        isTodaySelected: widget.daySelected,
        isWeekSelected: widget.isWeekSelected,
        isMonthSelected: widget.isMonthSelected,
        isWeeklySelected: widget.isWeeklySelected,
        isGroupSelected: widget.isGroupSelected,
        isSettingsSelected: widget.isSettingsSelected,
    ),
      body: SfCalendar(
